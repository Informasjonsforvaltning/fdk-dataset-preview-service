...
internal enum class PreviewFormat {
    ...
    companion object {
        ...
        fun detect(contentType: String?, resourceName: String): PreviewFormat? = when {
        ...
        fun fromFileName(fileName: String): PreviewFormat? = when {
        ...
        private fun isCsvContentType(contentType: String?): Boolean = contentType != null &&
            (
                CSV_CONTENT_TYPE_REGEX.containsMatchIn(contentType) ||
                    EXCEL_CONTENT_TYPE_REGEX.containsMatchIn(contentType)
                )

        private fun isPlainContentType(contentType: String?): Boolean = contentType != null &&
            (
