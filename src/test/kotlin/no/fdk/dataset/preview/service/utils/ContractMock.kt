...
fun startMockServer() {
    if(!mockserver.isRunning) {
        ...
        mockserver.stubFor(get(urlEqualTo("/download/test.xls"))
            .willReturn(aResponse()
                .withStatus(200)
                .withHeader("Content-Type", "application/vnd.ms-excel")
                .withBody(ObjectFactory::class.java.classLoader
                    .getResourceAsStream("test.xls")?.readAllBytes())
            )
        )
        mockserver.stubFor(get(urlEqualTo("/download/xls-zip"))
            .willReturn(aResponse()
                .withStatus(200)
                .withHeader("Content-Type", "application/zip")
                .withBody(ObjectFactory::class.java.classLoader
                    .getResourceAsStream("test.xls.zip")?.readAllBytes())
            )
        )
